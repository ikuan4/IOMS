...
@section('content')
    <div class="header-card">
        <div class="header-left">
            <h2>Edit Ticket</h2>
            <p class="muted">Update ticket details.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('tickets.update', $ticket) }}" enctype="multipart/form-data" data-ticket-attachments-form>
        @csrf
        @method('PUT')
        ...
        <div class="card" style="margin-top:16px;">
            <div style="display:flex;flex-direction:column;gap:18px;max-width:800px;">
                ...
                <div>
                    ...
                    <select name="ticket_type_id" id="ticket_type_id" required style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;">
                        @foreach($ticketTypes as $type)
                            <option value="{{ $type->id }}" {{ (string) old('ticket_type_id', $ticket->ticket_type_id) === (string) $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="ticket_module_id" id="ticket_module_id" required style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;">
                        @foreach($ticketModules as $module)
                            <option value="{{ $module->id }}" {{ (string) old('ticket_module_id', $ticket->ticket_module_id) === (string) $module->id ? 'selected' : '' }}>{{ $module->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="subject" id="subject" value="{{ old('subject', $ticket->subject) }}" required style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;" />
                    @error('subject')
                        <div style="color:#dc2626;font-size:13px;margin-top:6px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:14px;">
                    ...
                    <div>
                        ...
                        <select name="assigned_to" id="assigned_to" style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;">
                            <option value="" {{ old('assigned_to', $ticket->assigned_to) ? '' : 'selected' }}>Unassigned</option>
                            @foreach($assignees as $u)
                                <option value="{{ $u->id }}" {{ (string) old('assigned_to', $ticket->assigned_to) === (string) $u->id ? 'selected' : '' }}>{{ $u->name }}{{ $u->email ? ' (' . $u->email . ')' : '' }}</option>
                            @endforeach
                        </select>
                        @error('assigned_to')
                            <div style="color:#dc2626;font-size:13px;margin-top:6px;">{{ $message }}</div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            name="due_at"
                            id="due_at"
                            value="{{ old('due_at', $ticket->due_at ? $ticket->due_at->format('Y-m-d') : '') }}"
