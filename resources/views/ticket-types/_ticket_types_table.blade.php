<table style="width:100%; border-collapse:collapse;">
    ...
    @forelse ($ticketTypes as $type)
        ...
        <tr style="border-bottom:1px solid #f3f4f6; height:62px; {{ $isDeleted ? 'opacity:1;' : '' }}">
            ...
            @endif
            <td style="padding:8px;">{{ Str::limit($type->description ?? 'No description', 70) }}</td>
            ...
            <td style="padding:8px; text-align:right; white-space:nowrap;">
                @if(!$isDeleted)
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('update', $type))
                        <a href="{{ route('ticket-types.edit', $type) }}" style="background:#e0f2fe;color:#0369a1;padding:10px 12px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;border:none;cursor:pointer;margin-right:6px;text-decoration:none;" title="Edit ticket type"><span data-feather="edit"></span></a>
                    @endif
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('delete', $type))
                        <form action="{{ route('ticket-types.destroy', $type) }}" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'delete', title: 'Delete Ticket Type', subtitle: 'This will soft delete the ticket type', message: 'Are you sure you want to delete {{ addslashes($type->name) }}?', confirmText: 'Delete Ticket Type', checkDependenciesUrl: '{{ route('ticket-types.check_delete_dependencies', $type) }}', form: this});">
                            @csrf
                            @method('DELETE')
                ...
                @else
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('restore', $type))
                        <form action="{{ route('ticket-types.restore', $type->id) }}" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'restore', title: 'Restore Ticket Type', subtitle: 'This will restore the ticket type', message: 'Are you sure you want to restore {{ $type->name }}?', confirmText: 'Restore Ticket Type', form: this});">
                            @csrf
...
<div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
    <div style="flex:1; min-width:160px; color:var(--muted,#6b7280); font-weight:600;">
        Total Ticket Types: {{ $ticketTypes->total() }}
    </div>
