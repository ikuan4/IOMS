...
@section('content')
    ...
    <div style="margin-top:12px;">
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:16px;">
            @foreach ($cards as $key => $card)
                ...
                <a href="{{ route('ticket-types.index', $params) }}" style="text-decoration:none;color:inherit;">
    ...
    <div class="header-right" style="display:flex;gap:12px;align-items:flex-end; margin-top:16px; flex-wrap:wrap;">
        <form method="GET" action="{{ route('ticket-types.index') }}" id="ticketTypeSearchForm" style="display:flex;gap:12px;align-items:flex-end; flex-wrap:wrap;">
            <input type="hidden" name="status" value="{{ $status ?? 'all' }}">
            <input type="hidden" name="per_page" id="searchPerPage" value="{{ request()->query('per_page', 10) }}">
            <input type="text" name="search" id="ticketTypeSearchInput" value="{{ $search }}" placeholder="Search by name or description..." oninput="debouncedTicketTypeSearch()" style="padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;min-width:330px;width:330px;font-size:15px;" />
        </form>

        @if(auth()->user()->isSuperAdmin() || auth()->user()->can('create', \App\Models\TicketType::class))
            <a href="{{ route('ticket-types.create') }}" style="background:#22c55e;color:white;padding:10px 24px;border-radius:10px;font-weight:1000;width:220px;display:flex;justify-content:center;align-items:center;gap:8px;white-space:nowrap;text-decoration:none;">
    ...
    <div class="card" style="margin-top:12px;overflow:hidden;">
        <div id="ticketTypesTableWrapper">
            @include('ticket-types._ticket_types_table')
        </div>
    </div>
