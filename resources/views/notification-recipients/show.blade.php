...
@section('content')
    ...
    <div class="card" style="margin-top:16px;">
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(300px, 1fr));gap:24px;">
            ...
            <div>
                ...
                <div style="font-size:16px;">{{ $recipient->creator->name ?? 'N/A' }} on {{ $recipient->created_at->timezone('Asia/Kolkata')->format('d M Y, h:i A') }}</div>
            </div>

            <div>
                <div style="font-size:13px;font-weight:600;color:#6b7280;margin-bottom:6px;">LAST UPDATED BY</div>
                <div style="font-size:16px;">{{ $recipient->updater->name ?? 'N/A' }} on {{ $recipient->updated_at->timezone('Asia/Kolkata')->format('d M Y, h:i A') }}</div>
            </div>
        </div>
    </div>
    ...
    <div style="margin-top:20px;display:flex;gap:12px;">
        @can('notification-recipients.edit')
            <a href="{{ route('notification-recipients.edit', $recipient->id) }}" class="btn" style="background:#0B6BBD;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:500;">
                Edit Recipient
            </a>
        @endcan
