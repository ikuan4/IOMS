...
@section('content')
    ...
    <div class="card" style="margin-top:16px;padding:20px;">
        <form method="GET" action="{{ route('audit-logs.index') }}" id="auditLogSearchForm" style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;">
            ...
            <div style="flex:0 0 auto;min-width:260px;">
                ...
                >
                    ...
                    @foreach($options as $value => $label)
                        <option value="{{ $value }}" {{ ((string) $value === $currentAuditableType) ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div style="flex:0 0 auto;min-width:260px;max-width:520px;">
                ...
                >
                    ...
                    @endphp
                    @foreach($uOptions as $value => $label)
                        <option value="{{ $value }}" {{ ((string) $value === $currentUserId) ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <input type="hidden" name="per_page" id="per_page_hidden" value="{{ request()->query('per_page', 20) }}">
        </form>
    </div>

    <div class="card" style="margin-top:12px;overflow:hidden;">
        <div id="auditLogsTableWrapper">
            @include('audit-logs._audit_logs_table')
        </div>
    </div>
