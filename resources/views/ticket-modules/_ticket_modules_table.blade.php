<table style="width:100%; border-collapse:collapse;">
    ...
    <tbody>
    @forelse ($ticketModules as $module)
        @php $isDeleted = !is_null($module->deleted_at); @endphp
        <tr style="border-bottom:1px solid #f3f4f6; height:62px; {{ $isDeleted ? 'opacity:1;' : '' }}">
            <td style="padding:8px;">{{ $module->name }}</td>
            @if(auth()->user() && auth()->user()->isSuperAdmin())
                <td style="padding:8px;">{{ $module->branch->name ?? 'N/A' }}</td>
            @endif
            <td style="padding:8px;">{{ Str::limit($module->description ?? 'No description', 70) }}</td>
            ...
            <td style="padding:8px; text-align:right; white-space:nowrap;">
                @if(!$isDeleted)
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('update', $module))
                        <a href="{{ route('ticket-modules.edit', $module) }}" style="background:#e0f2fe;color:#0369a1;padding:10px 12px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;border:none;cursor:pointer;margin-right:6px;text-decoration:none;" title="Edit ticket module"><span data-feather="edit"></span></a>
                    @endif
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('delete', $module))
                        <form action="{{ route('ticket-modules.destroy', $module) }}" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'delete', title: 'Delete Ticket Module', subtitle: 'This will soft delete the ticket module', message: 'Are you sure you want to delete {{ addslashes($module->name) }}?', confirmText: 'Delete Ticket Module', checkDependenciesUrl: '{{ route('ticket-modules.check_delete_dependencies', $module) }}', form: this});">
                            @csrf
                            @method('DELETE')
                    ...
                    @endif
                @else
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->can('restore', $module))
                        <form action="{{ route('ticket-modules.restore', $module->id) }}" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'restore', title: 'Restore Ticket Module', subtitle: 'This will restore the ticket module', message: 'Are you sure you want to restore {{ $module->name }}?', confirmText: 'Restore Ticket Module', form: this});">
                            @csrf
...
<div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
    <div style="flex:1; min-width:160px; color:var(--muted,#6b7280); font-weight:600;">
        Total Ticket Modules: {{ $ticketModules->total() }}
    </div>
