...
@section('title', 'Edit Ticket Module')

@section('content')
    <div class="header-card">
        <div class="header-left">
            <h2>Edit Ticket Module</h2>
            <p class="muted">Update ticket module details.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('ticket-modules.update', $ticketModule) }}">
        @csrf
        @method('PUT')
            ...
            <div style="display:flex;flex-direction:column;gap:18px;max-width:600px;">
                ...
                <div>
                    ...
                    <input
                        ...
                        name="name"
                        id="name"
                        value="{{ old('name', $ticketModule->name) }}"
                        required
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="4"
                        style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;"
                    >{{ old('description', $ticketModule->description) }}</textarea>
                    @error('description')
                        <div style="color:#dc2626;font-size:13px;margin-top:6px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <label class="toggle-switch" style="display:inline-flex;align-items:center;cursor:pointer;gap:12px;">
                        <input
                            ...
                            name="is_active"
                            value="1"
                            {{ old('is_active', $ticketModule->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
                            class="toggle-input"
                        >
        ...
        <div style="margin-top:20px;display:flex;gap:12px;">
            ...
            <a href="{{ route('ticket-modules.index') }}" class="btn" style="background:#6b7280;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;font-size:15px;font-weight:500;">
                Cancel
            </a>
        </div>
    </form>
