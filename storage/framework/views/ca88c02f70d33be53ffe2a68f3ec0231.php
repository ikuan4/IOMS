<table style="width:100%; border-collapse:collapse;">
    ...
    <tbody>
    <?php $__empty_1 = true; $__currentLoopData = $ticketModules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php $isDeleted = !is_null($module->deleted_at); ?>
        <tr style="border-bottom:1px solid #f3f4f6; height:62px; <?php echo e($isDeleted ? 'opacity:1;' : ''); ?>">
            <td style="padding:8px;"><?php echo e($module->name); ?></td>
            <?php if(auth()->user() && auth()->user()->isSuperAdmin()): ?>
                <td style="padding:8px;"><?php echo e($module->branch->name ?? 'N/A'); ?></td>
            <?php endif; ?>
            <td style="padding:8px;"><?php echo e(Str::limit($module->description ?? 'No description', 70)); ?></td>
            ...
            <td style="padding:8px; text-align:right; white-space:nowrap;">
                <?php if(!$isDeleted): ?>
                    <?php if(auth()->user()->isSuperAdmin() || auth()->user()->can('update', $module)): ?>
                        <a href="<?php echo e(route('ticket-modules.edit', $module)); ?>" style="background:#e0f2fe;color:#0369a1;padding:10px 12px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;border:none;cursor:pointer;margin-right:6px;text-decoration:none;" title="Edit ticket module"><span data-feather="edit"></span></a>
                    <?php endif; ?>
                    <?php if(auth()->user()->isSuperAdmin() || auth()->user()->can('delete', $module)): ?>
                        <form action="<?php echo e(route('ticket-modules.destroy', $module)); ?>" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'delete', title: 'Delete Ticket Module', subtitle: 'This will soft delete the ticket module', message: 'Are you sure you want to delete <?php echo e(addslashes($module->name)); ?>?', confirmText: 'Delete Ticket Module', checkDependenciesUrl: '<?php echo e(route('ticket-modules.check_delete_dependencies', $module)); ?>', form: this});">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="submit" style="background:#fee2e2;color:#b91c1c;padding:10px 12px;border-radius:8px;display:flex;align-items:center;justify-content:center;border:none;cursor:pointer;" title="Soft delete ticket module"><span data-feather="trash-2"></span></button>
                        </form>
                    <?php endif; ?>
                <?php else: ?>
                    <?php if(auth()->user()->isSuperAdmin() || auth()->user()->can('restore', $module)): ?>
                        <form action="<?php echo e(route('ticket-modules.restore', $module->id)); ?>" method="POST" style="display:inline-block;" onsubmit="event.preventDefault(); showConfirmModal({type: 'restore', title: 'Restore Ticket Module', subtitle: 'This will restore the ticket module', message: 'Are you sure you want to restore <?php echo e($module->name); ?>?', confirmText: 'Restore Ticket Module', form: this});">
                            <?php echo csrf_field(); ?>
                            <?php if(!empty($search)): ?><input type="hidden" name="search" value="<?php echo e($search); ?>"><?php endif; ?>
                            <button type="submit" style="background:#dcfce7;color:#15803d;padding:10px 12px;border-radius:8px;display:flex;align-items:center;justify-content:center;border:none;cursor:pointer;" title="Restore ticket module"><span data-feather="rotate-ccw"></span></button>
                        </form>
                    <?php endif; ?>
                <?php endif; ?>
            </td>
    ...
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        <tr><td colspan="5" style="padding:12px;">No ticket modules found.</td></tr>
    <?php endif; ?>
    </tbody>
</table>
...
<div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
    <div style="flex:1; min-width:160px; color:var(--muted,#6b7280); font-weight:600;">
        Total Ticket Modules: <?php echo e($ticketModules->total()); ?>

    </div>

    <div style="flex:1; display:flex; justify-content:center;">
        <?php
            $current = $ticketModules->currentPage();
            $last = $ticketModules->lastPage();
    ...
    <div style="flex:1; min-width:180px; display:flex; justify-content:flex-end; align-items:center; gap:8px;">
        <?php
            $currentPerPage = (int) request()->query('per_page', $ticketModules->perPage() ?? 10);
        ?>
        <form method="GET" action="<?php echo e(url()->current()); ?>" id="perPageForm" style="display:flex;align-items:center;gap:8px;">
            ...
            <select name="per_page" id="per_page" onchange="ajaxFetchTicketModules(1)" style="padding:8px;border-radius:8px;border:1px solid var(--muted,#e5e7eb);background:var(--card);color:var(--text,inherit);">
...
<?php /**PATH E:\xampp\htdocs\IOMS\resources\views/ticket-modules/_ticket_modules_table.blade.php ENDPATH**/ ?>