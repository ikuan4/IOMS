...
<div class="card" style="margin-top:16px;">
    <div class="user-form-row" style="gap:18px;">
        ...
        <div>
            ...
            <input id="name" name="name" type="text" autocomplete="name" required value="<?php echo e(old('name', $user->name)); ?>" style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;" >
            <?php $__errorArgs = ['name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><div style="color:#dc2626;font-size:13px;"><?php echo e($message); ?></div><?php unset($message);
...
unset($__errorArgs, $__bag); ?>
        ...
        <div>
            ...
            <input id="email" name="email" type="email" value="<?php echo e(old('email', $user->email)); ?>" autocomplete="email" required style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div>
            <label for="password" style="font-size:15px;font-weight:600;">Password <span style="color:#dc2626;margin-left:2px;">*</span> <?php if($user->exists): ?><span class="muted" style="font-weight:400;">(leave blank to keep)</span><?php endif; ?></label><br>
            <input id="password" name="password" type="password" autocomplete="new-password" <?php echo e(!$user->exists ? 'required' : ''); ?> style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;" >
...
unset($__errorArgs, $__bag); ?>
        ...
        <div>
            ...
            <input id="password_confirmation" name="password_confirmation" type="text" autocomplete="new-password" <?php echo e(!$user->exists ? 'required' : ''); ?> style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;">
        </div>
        ...
        <div>
            ...
            <select id="role_id" name="role_id" required style="width:100%;padding:14px 16px;border-radius:10px;border:1px solid #d0d7e0;font-size:15px;background:white;">
                ...
                <?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    ?>
                    <?php if($isProtectedRole && !$currentIsSuperAdmin): ?>
                        <?php continue; ?>
                    <?php endif; ?>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div>
            ...
            <label class="toggle-switch">
                <input id="active" name="active" type="checkbox" value="1" <?php echo e(old('active', $user->active ?? true) ? 'checked' : ''); ?>>
        ...
        <?php if($user->exists): ?>
            <hr>
            <div>
                <div class="muted" style="margin-bottom:10px;font-size:14px;">Email bounce count: <strong><?php echo e($user->email_bounce_count); ?></strong><br>Last bounced (IST): <strong><?php if($user->email_bounced_at): ?><?php echo e($user->email_bounced_at->timezone('Asia/Kolkata')->format('d M Y, H:i')); ?><?php else: ?> Never <?php endif; ?></strong></div>
                <div>
                    <input type="hidden" name="reset_bounce" value="0">
                    <label class="toggle-switch">
                        <input id="reset_bounce" name="reset_bounce" type="checkbox" value="1" <?php echo e(old('reset_bounce') ? 'checked' : ''); ?>>
                        <span class="toggle-slider"></span>
        ...
        <div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;">
            ...
            <a href="<?php echo e(route('users.index')); ?>" style="background:#f51607;color:rgb(253,253,253);padding:14px 24px;border-radius:10px;font-weight:1000;font-size:15px;text-decoration:none;display:flex;align-items:center;gap:8px;"><span data-feather="x-circle"></span> Cancel</a>
        </div>
        ...
        <div class="user-form-right" style="min-width:260px;display:flex;align-items:stretch;justify-content:center;flex:1 1 420px;">
            <div style="padding:8px;border-radius:10px;background:var(--card, #fff);text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;width:420px;box-sizing:border-box;margin:0 auto;">
                ...
                <div id="avatarPreviewContainer" style="width:320px;height:320px;margin:0 auto;border-radius:6px;overflow:hidden;display:flex;align-items:center;justify-content:center;background:var(--muted-bg,#f3f4f6);">
                    <img id="avatarPreview" src="<?php echo e(old('avatar_preview', $user->avatar ? asset('storage/'.$user->avatar) : '')); ?>" alt="<?php echo e($user->name); ?> avatar" style="max-width:100%;max-height:100%;display:block;object-fit:cover;" />
                ...
                <div style="margin-top:12px;display:flex;flex-direction:column;align-items:center;gap:8px;">
                    ...
                    <div id="avatarFilename" style="font-size:13px;color:var(--muted,#6b7280);<?php echo e($user->avatar ? 'display:block;' : 'display:none;'); ?>" data-initial-filename="<?php echo e($user->avatar ? basename($user->avatar) : ''); ?>"><?php echo e($user->avatar ? basename($user->avatar) : ''); ?></div>
                </div>
                ...
                try {
                    ...
                    fetch(form.action, {
                        ...
                        return resp.text().then(function(txt){
                            ...
                            if (resp.ok) { window.location = "<?php echo e(route('users.index')); ?>"; return; }
...
<?php /**PATH E:\xampp\htdocs\IOMS\resources\views/users/_form.blade.php ENDPATH**/ ?>